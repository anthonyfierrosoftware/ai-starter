...
const ContentCard = ({ children, heading, subheading, isCentered=false, ...props }) => {
  return (
    <FlexColumn className="content-card" style={{alignItems: isCentered && "center"}} gap={16}>

        {heading && <Heading style={{textAlign: isCentered && "center", marginBottom: "16px"}}>{heading}</Heading>}    
