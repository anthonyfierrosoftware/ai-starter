    ...
    def post(self, request):
        ...
        try:
            try:
                ...
                return generate_response(status=200, data=None, custom_message="Email Found")
            except Exception as e:
                return generate_response(status=400, data="Email not found", custom_message="A user account with this email does not exist")
        except Exception as e:
            print(f"error sending verification email:{e}")  
            return generate_response(status=500, data=str(e)+".", custom_message="Could not check email")
    ...
    def post(self, request):
        ...
        try:
            ...
            if not request.user.is_authenticated:
                return generate_response(status=403, data=None, custom_message="User not authenticated")            
     
            if user.profile.verified:
                return generate_response(status=200, data=UserResponseSerializer(user).data, custom_message="Email is already verified")
            ...
            if emailVerificationCode(request, user, valid_pass):                   
                return generate_response(status=201, data=UserResponseSerializer(user).data, custom_message="Email Sent")
            
            return generate_response(status=500, data=None, custom_message="Verification email was unable to send: please check email config.")


            
        except Exception as e:
            print(f"error sending verification email:{e}")  
            return generate_response(status=500, data=str(e)+".", custom_message="Could not send validation email")
    ...
    def post(self, request):
        ...
        try:
            ...
            if not user.profile.emailVerificationCode:
                    return generate_response(status=412, data=None, custom_message="User has not requested a verification email yet: No Code")
                    
            if user.profile.verified:
                return generate_response(status=202, data=UserResponseSerializer(user).data, custom_message="Email is already verified")
            ...
            try:
                ...
                if str(hash.hexdigest()) == str(user.profile.emailVerificationCode):  
                    ...
                    user.profile.save()           
                    return generate_response(status=200, data=UserResponseSerializer(user).data, custom_message="user Verified")
                
                return generate_response(status=406, data=error_list_object("Verification Code", "Code did not match."), custom_message="Code mismatch")
            except Exception as e:
                print(f"error processing verification code:{e}")  
                return generate_response(status=422, data=error_list_object("Verification Code","Could not retrieve verification code."), custom_message=str(e))

            
        except Exception as e:
            print(f"error confirming user verification:{e}")  
            return generate_response(status=500, data=str(e)+".", custom_message="Could not confirm user validation.")
    ...
    def post(self, request):
        ...
        try:
            ...
            try:
                ...
                return generate_response(status=403, data=None, custom_message=f"Error retrieving user from email: {e}.")
            ...
            # send email
            if emailVerificationCode(request, user, valid_pass):                   
                return generate_response(status=201, data=None, custom_message="Email Sent")
