...
from users.serializers.profile_serializers import ProfileRegisterSerializer, ProfileUpdateSerializer
...
class UserUpdateSerializer(serializers.ModelSerializer):
    ...
    def validate(self,data):
        ...
        if "username" in data and User.objects.annotate(username_lower=Lower('username')).filter(username_lower=(data["username"]).lower()).exclude(username=instance.username if instance and instance.username else None).exists():
            error_dict["username"] = "This username is taken."


        if "email" in data and  User.objects.filter(email__iexact=data["email"]).exclude(email=instance.email if instance and instance.email else None).exists():
