        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    return generate_response(status=400, data="Error parsing data please refer to custom message for details.", custom_message=str(e))
            ...
            try:
                ...
                return generate_response(status=400, data="Could not find prompt.", custom_message="No Prompt")
            ...
            try:
                ...
                if "conversation" in data and int(data["conversation"]):
                    conversation = apps.get_model("aiModule.Conversation").objects.get(pk=int(data["conversation"]))
                    
                    #check for config update then update config or create config if there is none
                    if "config" in data:
                        try:
                            if conversation.llmConfig:
                                config_serializer= ConfigSerializer(conversation.llmConfig, data=data["config"], partial=True)
                            else:
                            ...
                            if config_serializer.is_valid():
                                ...
                                for err in config_serializer.errors:   
                                    try:
                                        ret.append(error_list_object(err, str(config_serializer.errors[err][0])))
                                    except Exception as e:
                                        print(f"error processing serializer errors: {e}")
                                        
                                return generate_response(status=422, data=ret, custom_message="Invalid configuration, if you are trying to update the configuration please refer to docs; otherwise remove the config object to send messages using current llmConfig")
                            
                        except Exception as e:
                            print(f"Error processing configuration in ChatComplete: {e}")
                            return generate_response(status=400, data="Error serializing update to configuration", custom_message=f"Error: {e}")
                else:
                    
                    if not "config" in data or "llm" not in data["config"]:
                        return generate_response(status=400, data="config required to start a conversation, must include at least: llm.", custom_message=None)
                    ...
                    #create conversation
                    conversation = apps.get_model("aiModule.Conversation").objects.create(
                    ...
                    try:
                        ...
                        conversation.delete()
                        return generate_response(status=400, data="Error creating config.", custom_message=f"Error: {e}")
                    ...
                    try:
                        ...
                        conversation.delete()
                        config.delete()
                        return generate_response(status=400, data="Error updating conversation.", custom_message=f"Error: {e}")
            ...
            if not conversation.llmConfig:
                ...
                return generate_response(status=400, data="Conversation has no config object, please send a config with this conversation to continue", custom_message=None)
                
            try:
                # please reference llmConfig.py for list of available LLMs
                match conversation.llmConfig.llm:
                    case "OPEN_AI":
                        llm = OpenAIWrapper( api_key= conversation.llmConfig.apiKey, chat_model= conversation.llmConfig.chat_model,system_instructions=conversation.llmConfig.systemInstructions)
                    case "CLAUDE":
                        llm = ClaudeWrapper( api_key= conversation.llmConfig.apiKey, chat_model= conversation.llmConfig.chat_model,system_instructions=conversation.llmConfig.systemInstructions)
                    case "MISTRAL":
                        llm = MistralWrapper( api_key= conversation.llmConfig.apiKey, chat_model= conversation.llmConfig.chat_model,system_instructions=conversation.llmConfig.systemInstructions)
                    case "HUGGINGFACE":
                        llm = HuggingWrapper( api_key= conversation.llmConfig.apiKey, chat_model= conversation.llmConfig.chat_model,system_instructions=conversation.llmConfig.systemInstructions)
                ...
                if llm.authorize():
                    ...
                    if conversation.chatHistory:
                        llm.set_conversation_history(json.loads(conversation.chatHistory))
                    
                    temp, success = llm.send_text_chat(message=prompt, system_instructions=conversation.llmConfig.systemInstructions)
                    
                    
                    if not success:
                        return generate_response(status=400, data=str(temp), custom_message="Error retrieving chat, please refer to logs.")
                    # update and save converstaion conversation.chatHistory=json.dumps(temp["chatHistory"])
                    ch = temp["chatHistory"]
                    if not ch[0]["role"] == "system":
                        ch.insert( 0, {"role": "system", "content": conversation.llmConfig.systemInstructions})
                    ...
                    match conversation.llmConfig.llm:
                        ...
                        case "HUGGINGFACE":
                            if conversation.llmConfig.chat_model == "meta-llama/Llama-2-70b-chat-hf":
                    ...
                    return generate_response(status=403, data="Invalid Credentials", custom_message=None)
                ...
                return generate_response(status=500, data=str(e)+".", custom_message="Error generating chat completion")
