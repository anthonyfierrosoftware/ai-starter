...
class OpenAIWrapper(AbstractWrapper):
    ...
    def set_conversation_history(self,ch= None):
        try:
            ...
            print(f"Error checking system config in openaiwrapper set_conversation_history: {e}")
    ...
    def authorize(self):
        ...
        try:
            ...
            print(e)
            print("Apologies big dog, we cant connect right now, try checking your env file ;)")
            return 0
    ...
    def text_chat(self, messages = [], model_id = None, results_num=1,  temp=1, top_p = 1):
        ...
        try:
            ...
            if max_tokens > self.max_tokens:
                print(f"warning this request has surpassed the maximum tokens: {max_tokens}, limited to: {self.max_tokens} results may be cut short, you may want to reset the chat")
    ...
    def send_text_chat(self, message='', system_instructions="You are a helpful assistant", model_id = None, reset=False, results_num=1,  temp=1, top_p = 1):
        '''
        ...
        try:
            ...
            response, success = self.text_chat(messages = self.conversation_history[-self.msg_limit:] ,model_id = model_id, results_num=results_num, temp=temp, top_p=top_p)
