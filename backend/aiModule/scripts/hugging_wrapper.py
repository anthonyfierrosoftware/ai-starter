...
class HuggingWrapper(AbstractWrapper):
    def __init__(self, api_key = None, chat_model=None, system_instructions=None):
        ...
        self.client = HuggingFaceEndpoint(repo_id= self.chat_model, huggingfacehub_api_token=self.api_key)
        ...
        self.system_instructions= system_instructions if system_instructions else  "You are a helpful assistant"
    ...
    def set_chat_model(self,model = "meta-llama/Llama-2-70b-chat-hf"):
        ...
        try:
            self.client = HuggingFaceEndpoint(repo_id= model, huggingfacehub_api_token=self.api_key)
            self.model = model
            return (None,True)
        
        except Exception as e:
            self.client = HuggingFaceEndpoint(repo_id= self.model, huggingfacehub_api_token=self.api_key)
    ...
    def authorize(self):
        ...
        try:
            ...
            print(e)
            print("Apologies big dog, we cant connect right now, try checking your api key ;)")
            return 0
    ...
    def text_chat(self, data = {}, prompt= None, model_id = None, temp=0.8, top_p = 0.95):
        ...
        try:
            ...
            self.client = HuggingFaceEndpoint(repo_id= self.chat_model, huggingfacehub_api_token=self.api_key, temperature=temp, top_p=top_p, max_new_tokens=self.max_tokens )
    ...
    def send_text_chat(self, message='', system_instructions=None, model_id = None, reset=False,  temp=0.8, top_p = .95):
        '''
        ...
        try:
            ...
            self.conversation_history.append({"role":"user","content":f"{message}.\n"})  
            ...
            response, success = self.text_chat( model_id = model_id, prompt=prompt, temp=temp, top_p=top_p)
