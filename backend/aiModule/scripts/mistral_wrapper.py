...
class MistralWrapper(AbstractWrapper):
    ...
    def set_conversation_history(self,ch= None):
        try:
            ...
            print(f"Error checking system config in mistralwrapper set_conversation_history: {e}")
    ...
    def send_text_chat(self, message='', system_instructions=None, model_id = None, reset=False,  temp=0.7, top_p = 1):
        '''
        ...
        try:
            ...
            response, success = self.text_chat(messages = self.conversation_history[-self.msg_limit:], model_id = model_id, temp=temp, top_p=top_p)
