...
class ConfigSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if "llm" in data and "chat_model" not in data:
            error_dict["chat_model"] = (
                f"Please specify a chat_model when updating the llm, options: {str(LLM_MODEL)}"
            )

        if "chat_model" in data and "llm" not in data:
            error_dict["llm"] = (
                f"Please specify a llm when updating the chat_model, options: {str(LLM_MODEL)}"
            )
        ...
        if (
            ...
            and data["llm"] in LLM
            and data["chat_model"] not in LLM_MODEL[data["llm"]]
        ):
            error_dict["chat_model"] = (
                f"llm and chat_model do not match, options: {str(LLM_MODEL)}"
            )
